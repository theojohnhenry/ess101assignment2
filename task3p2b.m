...
PHI = [y_est(2:N_est-1), y_est(1:N_est-2), u_est(3:N_est), u_est(2:N_est-1)];
y_target = y_est(3:N_est); 

thetahat = (PHI' * PHI) \ (PHI' * y_target)

%% 1-step pred. RMSE 2.0337

PHI_Val = [y_val(2:N_val-1),y_val(1:N_val-2),u_val(3:N_val), u_val(2:N_val-1)];
y_val_target = y_val(3:end);
y_val_pred = PHI_Val * thetahat;

rmse_pred_b = sqrt(mean((y_val_target - y_val_pred).^2))
...
rmse_sim_b = sqrt(mean((y_val(3:end) - y_sim(3:end)).^2))
